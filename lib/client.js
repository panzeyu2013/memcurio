...
var MemcurioSettingsController = class {
  ...
  async reset(field) {
    const problem = routeProblem(field, void 0, this.face().value);
    if (problem) return this.fail(problem);
    this.busy = field;
    this.notify();
    try {
  ...
  async resetAll() {
    ...
    for (const field of pending) {
      if (routeProblem(field, void 0, this.face().value)) {
        this.busy = void 0;
        this.notify();
...
function MemcurioSettingsSection(props) {
  ...
  const commitText = (0, import_react.useCallback)(
    (field2, raw) => {
      const trimmed = raw.trim();
      if (trimmed === "") {
        void reset(field2).then(announce);
        return;
      }
      void save(field2, trimmed).then(announce);
    },
    [announce, reset, save]
  );
