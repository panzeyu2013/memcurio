...
1. **质量闸门**（每次提交可复跑，见 §4 运行卡）：485 tests / 28 files / 3027 expects / 0 fail（coverage 92.17% funcs / 93.76% lines）；`tsc --noEmit -p tsconfig.typecheck.json`（含 `client/`）；`biome lint src tests scripts client`；`bun run build` + `pack:check`（79 文件，dist/lib 反向校验 + `lib/client.js` loader 形态与 require 纯度校验）——全部在 CI 语义下可复现（仓库 CI 钉 bun 1.3.14、node >= 22.13 静态契约）。
...
```bash
PATH=/root/.bun/bin:$PATH /root/.bun/bin/bun test                  # 期望：485 pass / 28 files / 3027 expect / 0 fail
/root/.bun/bin/bun x tsc --noEmit -p tsconfig.typecheck.json       # 期望：exit 0（含 client/）
/root/.bun/bin/bun run lint                                        # 期望：Checked 80 files, no diagnostics
