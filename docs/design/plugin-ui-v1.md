...
> 状态：**设计基线（frozen）**，2026-09-05 第十六轮讨论定稿；**v1.9.4（指南行微调，第四十五轮）**：行标题简化为「记忆指南 / Memory guide」，折叠行只留标记+标题；字符数/工具数移入展开体**第一行**；锚点改为「system-prompt 卡片正上方」——镜像官方 `requestPromptAnchor` 规则（首 step 用 turn start、其后用 step start，再减一毫），与 `dsh-chamber-mcp` 的 registered-tools 行一致；**v1.9.3（Settings 文案精简，第四十四轮）**：移除记忆开关行的说明文本（"每个 agent step 都评估；内容未变不重复注入…"）——`injectContextNote` 从 en/zh 词典与 `settings/section.ts` 的开关行一并删除，locale 与面板断言同步；**v1.9.2（系统提示注入可见化，第四十三轮）**：read_path 指南进 system prompt 后该注入对用户是静默的——新增派生行 `memcurio-guide-injected`（`client/ui/guide-row.ts`）：扫描 harness 自有的 `system/message` 事件中的 `## memcurio memory` 段落并做 FNV 哈希，仅当签名与同类前驱 Context 不同时渲染一行（指南为常量 ⇒ 每会话最多一行；提示词不含该段落则一行不渲染）；行锚定在 system-prompt 卡片前一毫、沿用平台 disclosure 几何 + 书本主标记、展开即显示注入的指南正文；**不向会话写入任何东西**（0.1.5 下第三方私有 session event 会让日志不可打开，公开 append 路径无法置 `ignorable: true`），做法参考 `dsh-chamber-mcp` 的 MCP-tools 行；**v1.9.1（文本精简，第四十一轮）**：动态命中行去掉 `[memcurio] ` 前缀（块首一行 `Memory hits:`），单条命中空白折叠 + **220 字符截断**（`…`），摘要块改为一行标签 + `<<<MEMORY_SUMMARY`/`>>>MEMORY_SUMMARY` 短分隔；read_path 指南 3,387 → **1,554 字符**（契约不变）；引擎与工作台 simulator 共用同一 formatter（`renderHitLine`/`renderHitBlock`/`MAX_HIT_CHARS`），预览不再可能漂移；**v1.9（注入分层，第四十轮）**：read_path 使用指南不再随 user message 注入，改注册为 **system prompt section**（`memcurio-read-path`，order 2950，与工具 schema 同区，`renderReadPathInstructions()` 全文**无文件系统路径**、工具优先）；注入的 user message **只承载记忆内容本体**（`memory_summary.md` 摘要区块；空库不注入任何东西）；动态命中的检索 query 由"最近一条非插件用户文本"经去噪（代码块/URL/绝对路径/标记）+ 停用词处理生成，命中排序改为两遍打分（IDF + 多词短语奖励 + 内容去重 + 单文件 cap ≤3）；v1.1：存储附属 DSH home、语言政策、标题栏单按钮入口、参考版本声明；v1.2（S0 预查实证）：桥接通道对第三方关闭、标题栏槽位候选名、seed 模块表、挂载平面风险；v1.3（审查修订）：遥测开关（预览不计数）、usage 增量语义、审计 object、入口/通道措辞统一；v1.4（实现批次）：host 桥接层落定——store 注册表/事件打标点/审计尾+队列 diff/快照装配/写路径收据过滤、queue-updated 单 job 语义（实现面详见 §8.4；v1.4.1：A 类收口——桥集成测试、memory_read/shell 打点、evidence 源、雷达候选启发式、快照收据合成与 settings/dynamic 富化）；v1.6（G5/G6 先行批，**S0 依产品决定延期**）：同源传输（prefix 路由 snapshot+SSE+custom guard，§8.5）、客户端 snapshot/SSE+轮询降级、头部注入指示器、注入/写入 Toast、6 个 memory_* 工具行；`hostBridge` 默认开；v1.7（2026-09-16，产品指令）：记忆注入开关——会话头部在指示器右侧增加官方几何 Switch（36×20、`role=switch`，写同一 `injectContext` 字段），popover 首行同开关行；开关只放 Settings 面板首行（头部保持只读指示器），关闭态是独立状态（文字标注，不只靠颜色），拒绝写入在面板内以 locale key 报错；Settings 导航行标记——外壳 `navIcon` 按 id 硬编码图标、其余回退 General 齿轮，且 `settings.section` 选项只有 `{id, order, label, priority}`（无 icon 字段；实证版本 `dsh-client-ui-settings-general@0.1.5-rc.2`，rc.1 未复核），故由 `client/settings/nav-mark.ts` 给本行加 `data-memcurio-nav`、样式用书页 mask 绘制：只认设置对话框内、带外壳字形且文本等于本插件 label 的**唯一**候选行（歧义或缺失即不标，侧栏同名行不受影响），mask 不可用时回退外壳字形（上游具备 section icon 能力后删除该适配器）；v1.8（2026-09-16，产品指令）：**注入行标题**——插件注入的记忆消息在会话转录中不再沿用平台的通用标题「上下文注入/Context injection」，改由插件自己的行渲染为「记忆注入/Memory injection」。平台把每条插件注入的 `user/message` 交给同一个 keyed context 单元，标题由 `source.kind` 决定，durable source 上没有生产者标题/图标缝，因此由 `client/ui/context-row.ts` 以 priority -1 影子注册 `conversation.chat.node` 的 `context` 单元：仅 `source.kind === "plugin" && source.plugin === "@memcurio/dsh-plugin"` 走自有行，其余 context 节点（运行时快照 / agent instructions / skill catalog / notice / relay / 跨会话召回）从槽位账本取回被影子的 shipped 渲染器原样转发（自审 defensive fallback：账本探测失败或无 shipped 行时渲染最小 opaque 行，绝不丢节点）；前导字形改用 memcurio 自有书页标记（`MemoryMarkIcon`，`icons.ts`；适配器兜底的通用行仍用平台 ContextInjection 几何），行几何对齐 shipped disclosure row（24px 行 / 16px leading / 13px 标题 / 141px 代码块正文），bundle 运行期仍只 require `react`；上游在注入 source 上提供生产者标题/图标后删除该适配器；v1.8.1（2026-09-16，实机反馈修正）：Settings 面板——①「写回组合 base 值」视为 revert（清用户层条目，字段关闭又打开后「已覆盖/恢复默认」自动消解；路由对写回 base 同样清两半），不再是钉住等值覆盖，read-back 校验以「revert ⇒ 用户层条目缺失」为准；② Worker 路由行改为**整行堆叠控制组**（`memcurio-field-stack`：标签与说明占首行，两个 34px 输入弹性铺满 + 行内保存按钮占下一行），修复两个输入 + 说明 + 保存挤在同一行的实机问题；其余行仍为「左文案 + 右控件」紧凑行；v1.8.4（2026-09-16，E2E 修复批）：Phase-1 抽取回复被注入策略误判时**按行修复**（不再整条 dead-letter；全条仍不安全才拒绝），旧的策略 dead job 启动时一次性 requeue；worker 路由进程级回落 + blocked 5 分钟慢探；LLM 整合失败降级 rule provider（`consolidate.fallback`）；未应用 note 立即可搜（`pending`）；JSON 读取容忍控制字符与截断；v1.8.3（2026-09-16，产品指令）：6 个原生工具行的 leading 图标统一为 book 主标记（running/ok/error/interrupted 全部；终态只改标记颜色，不再让位状态点）；v1.8.2（2026-09-16，产品指令）：**静态注入拆分**——每个新会话恒注入 read-path 使用指南；`memory_summary.md` 仅在其内容非空时作为摘要区块一并注入（`renderStaticContext` 装配，engine 注入与 `staticContext` 预览共用），空库不再注入 `(memcurio memory not consolidated yet)` 占位符。本文是 S0 spike / M0 / M1 / M2 的唯一验收基准；实现与设计背离时先改本文再改代码。
...
- 记忆根 `~/.codex/memories/`：`MEMORY.md`（可 grep 的 Task Group 手册，由 Phase-2 agent 写）、`memory_summary.md`（首行 `v1`；**非空时每会话注入**，预算上限约 2500 token，内容含 User Profile / preferences / 主题索引与 "memory days" 新鲜度；空库时只注入 read-path 指南，不注入占位符 —— v1.8.2）、`raw_memories.md`（机械 Phase-1 合并）、`rollout_summaries/<id>.md`（含 cwd/rollout_path/updated_at/thread_id/lessons）、`skills/`、`extensions/`；git baseline + `phase2_workspace_diff.md` 驱动 diff 式整合。
...
**记忆开关（v1.7，2026-09-16 产品指令：开关只放 Settings）**：记忆注入的 ON/OFF **只在 Settings 面板**（`settings.section` 首行，官方 Switch 几何：36×20、16px thumb、`role="switch"` + `aria-checked` + 必填 label），写 `injectContext` 字段（同一 controller、写后回读校验、失败以 locale key 在面板内报错）；开关行**不带说明行**（v1.9.3，2026-09-16 产品指令：移除该说明文本——开关只需 label + switch，面板保持紧凑；pre-step 每个 step 评估 / 内容未变不重复注入属实现细节，不占界面）。会话头部**不承载任何 memcurio 面**（v1.7 产品指令，2026-09-16：顶部栏不再有记忆注入图标/开关/未读点）：开关与配置只在 Settings，注入与写入反馈仅经 Toast；注入预览（静态上下文 / read 指引 / 动态命中 / 预算条）与未读点保留在 `client/ui/injection-indicator.ts`（**有意不注册**，留给工作台 M0/M1 的状态面，入口注册即六行）；面板采用**紧凑行**布局（官方 General 偏好行范式：左文案 + 右控件，一行一项，说明折进行内第二行；Worker provider/model 合并为同一行的路由控件，编辑后由行内**保存按钮**提交，不做隐式失焦提交）——原先逐字段「标签行 + 控件行」把纵向空间翻倍。状态**只用图标**：标题行右侧 8px 状态点（chamber / dsh-chamber-mcp 约定：绿=就绪、灰=空闲/只读、红=错误、写入中脉冲；相位文案只在 title/aria-label，`prefers-reduced-motion` 下不动画），不再有「就绪」文字行；启用类开关（注册工具）的说明按实际语义写全（开启做什么、关闭影响什么、何时生效）；`hostBridge` 按产品决定连配置项一并删除（桥恒开：Config / settings 命名空间 / 面板三处都没有该字段），面板因此为五项设置。样式词汇对齐官方设置页（实证 rc.2：ui-settings-plugins/fields.module.css 的字段行 / 34px 输入 / 行内 reset、ui-settings-models/ModelsSection.module.css 的 16/24/500 标题与 14/22 说明、General 偏好行的开关行、平台 Switch 原子；--dsw-alias-label-error 在 rc.2 未定义，非法态改用 state-error-primary），只读态用 disabled、仅写入中用 readOnly（不伪装禁用）；关闭态是**独立状态**：指示器 `data-state="disabled"`、隐藏命中数与未读点、popover 明示"注入已关闭"（与"尚未注入"区分，且历史预览标注为关闭前值），状态不只靠颜色。S0 预查（v1.2，rc.1 实证）：候选槽位为 **`conversation.session.header.actions`**（"标题相邻会话操作"，ui-conversation 声明）或全宽 `conversation.view` tab；`settings.section` 仅适合设置页。`/memory` 回退存疑：rc.1 无已验证的"客户端命令唤起 UI"机制（命令在 agent 侧执行）——回退路径待 S0 实测后修订（可能为 conversation.view tab 或设置页直达）。
...
| 配置 | `ctx.settings`（`memcurio` 命名空间） | scope/injectContext/registerTools/injectBudgetTokens/provider/model（root 只读；hostBridge 不是配置项，桥恒开） | `@deepseek-ai/dsh-settings` + `dsh-settings-file`（settings.yaml；见 §3.3） |
...
- **验证与残留**：boot token 下发、守卫矩阵、SSE 上限/回收与 `settings.section` 注册已在隔离真机验证通过（见 [verification-s0-web.md](../verification-s0-web.md)）；跨 store delta 归属（帧带 root，SSE 按根过滤）与 snapshot↔stream 窗口（`?after=` 重放）已落地。剩余：会话内头部入口需真实会话人工确认一次、chamber 网关代理链路（index 缓存 / SSE 透传）复测，以及完整工作台（三面一轴 / 意图草稿 / 时间线回链，M0/M1）。v1.7 新增面同样只到 jsdom 回归网，待真实 Web 人工确认：Settings 的记忆 ON/OFF 首行（只读/不可用态禁用、写入被拒时的面板内报错）、Settings 布尔开关行与输入控件样式、`settings.action` 探针驱动的导航行书页标记（外壳 navIcon 的实证版本为 rc.2；rc.1 行为未复核）。
