...
> Status: **Web half executed and passing (2026-09-15)** — boot-token delivery, route guards, SSE cap/reclaim and the real-Chromium `settings.section` render were verified in an isolated profile; see [../verification-s0-web.md](../verification-s0-web.md). The only unverified item left is the session-header entry (needs a live conversation). Original status: **draft for the S0 run** (spike = plan + verification, no product code). Baseline: `docs/design/plugin-ui-v1.md` (in-tree **v1.5**, landed in rounds 25–26 - the `0077aaa` commit is the round-18 host-bridge record where the doc was still v1.4; re-read before the run). Upstream artifacts: the ledger was verified against the local DSH npm `0.1.2-rc.1` install tree; the RUN target is npm `latest` `0.1.5-rc.1` (re-verify every ledger item there — design §"参考版本声明").
>
...
| L17 | Web profile runs the agent plane behind **agent presets**: per-session composition from `agent.cordis.yml`; user presets under `$DSH_HOME/.agent-presets`; the shipped `standard` preset is the default; memcurio’s patch row (`id: memcurio`, `name: @memcurio/dsh-plugin`, `inject: [tools, llm, sessions, settings]` since round 27; the rc.1-era list lacked `settings`) is root-plane and may not resolve those services in the web profile (tools/llm moved behind presets there) | `dsh-agent-presets/README.md`; `dsh-web-app/cordis.patch.yml` (agent-plane section ~308+, preset roster ~433-441); memcurio `cordis.patch.yml` | VERIFIED(rc.1) — mounting strategy OPEN (P3) |
...
- **Mounting plane (P3) settled at the composition layer**: the composed tree places our row at the ROOT plane, right after `agent-presets`, and the root plane itself provides the injected services — `id: llm` (`@deepseek-ai/dsh-llm`), `id: tools` (`@deepseek-ai/dsh-tools`), `id: session` (`@deepseek-ai/dsh-session`), `id: settings` (`@deepseek-ai/dsh-settings-file`). The web-app layer only DISABLES concrete entries (`tool-bash`, `tool-pwsh`, `tool-jobs`, `tool-fs`, `tool-fs-search`, `agent-instructions`, `skill-*`); it does not relocate the service planes. So the L17 worry ("tools/llm moved behind presets in the web profile, the root-plane row may not resolve them") does NOT reproduce on 0.1.5-rc.1 — the root-plane insert is the right mounting point, and no preset-plane row is needed.
- Consequence to keep in mind for telemetry: because `tool-fs`/`tool-fs-search` are disabled in the web profile, DSH's built-in `read`/`grep`/`glob` never fire there; read-hit counting comes from this plugin's own `memory_read`/`memory_search` tools (its readTools/shellTools preset stays harmless).
