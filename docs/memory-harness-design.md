...
> 实现状态：M0–M4 代码全部落地（212 用例），真实 harness 验证待启用。现状图见 docs/architecture.md。
...
- 测试：bun test（212 用例）；类型检查 tsc --noEmit
...
### 5.3 验证方式
- 单测：存储/剪枝状态机/事务日志/检索（含 CJK）/MCP 协议/适配器事件流/安全/预算/策展——**已实现 212 用例**（bun test）
