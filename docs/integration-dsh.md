...
> Status: developer preview. This repository IS the plugin package `@memcurio/dsh-plugin` (engine + Cordis plugin + bundle manifest in one tarball), targeting the published DSH `0.1.5-rc.1` Cordis contracts (the current npm `latest`) while those contracts are pre-release. All non-DSH distribution surfaces (opencode/MCP/CLI) were removed in round 15.
