...
## 2. DB schema v5（index.sqlite）

迁移：schema_version = 5；`DROP TABLE IF EXISTS entries, fts, contradictions`（v4 及以下旧表）。
...
CREATE TABLE stage1_outputs(
  ...
  rollout_slug TEXT NOT NULL,        -- 文件名安全的 slug
...
```ts
stageUpsert(out: Stage1Output): void                       // INSERT OR REPLACE + audit
...
stageSelect(cfg: { maxUnusedDays: number; maxInputs: number }): Stage1OutputRow[]
  ...
  // 排序 usage_count DESC, 再 last_usage/generated_at 最近；截断 maxInputs；选中行标记 selected_for_phase2=1
...
export interface ConsolidatePlan {
  ...
  artifacts: Record<string, string>;  // 同步后预期的 MEMORY_DOCS + rollout_summaries/<slug>.md 内容
...
export class HttpLoopConsolidateProvider implements ConsolidateProvider {}
  ...
  // 循环上限 cfg.maxAgentSteps（默认 25）；写入的 rel 必须 .md 且在 workspace 内、content ≤ 256KB、secret 扫描（命中→redact）、注入扫描（命中→reject）；
  // 结束后合并所有 write_file 为 edits；校验 memory_summary 若存在首行须为 "v1"（违反 → 抛错回滚）。
...
export function runConsolidation(root: string, provider: ConsolidateProvider, opts: { execute: boolean }): Promise<{
  ...
  // execute=true：syncArtifacts → provider.consolidate → 校验 edits → 单 Transaction 内应用所有编辑（原子写 + audit("consolidate.apply", rel, report摘要)）
  //   → note 标记 applied → stageSelect 标记 selected → saveBaseline → audit("consolidate.done", "-", ...)
...
export class MemcurioAdapter {
  ...
  messageSeen(id, partId): Promise<void>                    // 计数（内存）
  toolExecuted(id, tool, {filePath?}): Promise<void>        // 计数 + 触碰文件收集
  sessionCompacted(id, summary?): Promise<void>             // 存内存 snapshot.summary（不落盘）
  sessionEnded(id): Promise<{staged: boolean}>              // 组装 RolloutSnapshot → stageSession（异步、异常吞掉记 log）→ 结束 sessions 行
...
- `session.deleted` → `adapter.sessionEnded(id)`；
- 移除 reflect 通道；Phase 1 抽取默认走 `HttpExtractProvider`（`MEMCURIO_LLM_*`，无 key 即 no-op；实现中没有保留内部会话通道）；
...
- SessionEnd → `sessionEnded`（触发 stageSession，用 `codexExecExtract` 通道或 HTTP）；
...
- db.test.ts（v5 迁移 + stage/note 方法）、config.test.ts、paths.test.ts、sanitize/budget/transaction/events/ids/llm 保持
- cli.test.ts（全部新命令 + exit codes + i18n）、mcp.test.ts（5 工具）、adapters.test.ts（engine 用 FakeExtractProvider 断言 stageSession 与注入）、codex.test.ts（socket 事件→staged 断言 + codexExecExtract 端到端）、opencode.test.ts（事件映射，默认 noop 抽取）、fixes.test.ts（repair/doctor 新语义）
- 删除：mdStore/retriever/safeSearch/prune/curate/compact/transfer/baseline/select 相关测试

## 9. 验收标准

1. `bun test` 全绿；`bun run typecheck`、`bun run lint` 干净；
...
3. 删除的旧文件无残留 import（typecheck 通过保证）。
