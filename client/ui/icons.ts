/**
 * Inline SVG marks of the memory UI (G5/G6).
 *
 * Conventions follow dsh-chamber-mcp's MCP row icon: 24-unit geometry drawn
 * for a 14px render, stroke 2, round caps/joins, a single `currentColor`, and
 * zero icon-package edge — the shipped bundle still requires only react, so
...
/** The 24-unit book-and-ribbon mark ("书页"), stroke geometry. */
const MEMORY_MARK_PATHS: readonly string[] = [
  "M5 6a3 3 0 0 1 3-3h11v18H8a3 3 0 0 1-3-3V6Z",
  "M5 18a3 3 0 0 1 3-3h11",
  "M10 3v6l2-1.5L14 9V3",
];

/** Platform context-injection geometry (16-unit, fill), inlined. */
const CONTEXT_INJECTION_PATHS: readonly string[] = [
...
/** One SVG string of the book mark (imperative toast host). */
export function memoryMarkSvg(size = 14): string {
...
/** One SVG string of the context-injection glyph (imperative toast host). */
...
export function MemoryMarkIcon({ size = 14 }: { size?: number }): ReactElement {
  return createElement(
    ...
    ...MEMORY_MARK_PATHS.map((d) => createElement("path", { key: d, d })),
  );
}

/** Platform context-injection glyph (inlined, 16-unit fill geometry). */
export function ContextInjectionIcon({ size = 14 }: { size?: number }): ReactElement {
  return createElement(
    "svg",
    { width: size, height: size, viewBox: "0 0 16 16", fill: "none", "aria-hidden": true, focusable: "false" },
    ...CONTEXT_INJECTION_PATHS.map((d) => createElement("path", { key: d, d, fill: "currentColor" })),
  );
}

/** Expand affordance of one tool row (the shipped downward chevron). */
export function ChevronDownIcon({ size = 14 }: { size?: number }): ReactElement {
  return createElement(
    "svg",
    {
      width: size,
