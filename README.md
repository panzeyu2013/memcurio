...
- **Ad-hoc notes** — `memory_remember` notes (kind `remember`|`forget`|`update`; written when the user asks or when the model confirms durable value) become append-only notes under `extensions/ad_hoc/notes/`, applied at the next consolidation.
...
- **Write: `memory_remember` / ad-hoc notes** — append-only notes under `extensions/ad_hoc/notes/` (file + SQLite row in one transaction, max 20,000 chars), written when the user asks or when the model confirms a durable preference, decision, correction or reusable lesson (optional `kind`: remember|forget|update). Secrets are redacted at write; promptware-injection payloads are rejected at the entry point with an audit record. Notes merge into `MEMORY.md` at the next consolidation. The model never edits memory files during a session; wrong or stale content is corrected by editing `MEMORY.md` directly or by the consolidation agent's own diff-driven cleanup.
...
- **Note writing** — memcurio authorizes a `memory_remember` note whenever the user asks **or** the model confirms durable value, and exposes `kind` (remember|forget|update). Codex's read-path fragment, its `ad_hoc_note` tool description and its phase prompts all gate notes on an explicit user request ("only when explicitly asked"; "do not write on your own"), and its note tool takes a model-supplied filename instead of a server-generated one.
