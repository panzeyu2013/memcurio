...
- **Read: search + progressive injection** — line-oriented lexical retrieval over `MEMORY.md`, `memory_summary.md`, `rollout_summaries/` and `skills/`, with read-time re-redaction and injection filtering. The summary is always injected budget-capped; dynamic hits (budget-derived, capped at 8) are injected per prompt, with CJK queries expanded into adjacent bigrams so Chinese prompts retrieve too. The read-path instructions tell the model when to call the memory tools and to cite what it used through the native `memory_cite` tool; assistant text is never parsed.
