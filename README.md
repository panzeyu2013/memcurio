...
**memcurio** is a memory and context management plugin for the [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (DSH). This repository is the single deliverable package `@memcurio/dsh-plugin`: a Cordis plugin (node half) that turns the harness's own session lifecycle into durable, workspace-scoped memories, injects them back into the agent loop, and registers seven native memory tools. The browser half (`dsh.client`) ships in this release: the **Settings panel**, the memory-injection transcript row, **injection/write toasts**, the system-prompt guide row, and custom transcript rows for the seven memory tools — served over a same-origin `/memcurio` snapshot + SSE route with automatic polling degradation. The session header deliberately carries no memcurio surface: the injection-indicator component stays unregistered for the future workbench status surface (see [docs/ui.md](docs/ui.md)). The full memory workbench over the store remains the next milestone (see [docs/todo.md](docs/todo.md)).
...
- **Ad-hoc notes** — explicit `memory_remember` notes (kind `remember`|`forget`|`update`) become append-only notes under `extensions/ad_hoc/notes/`, applied at the next consolidation.
- **Progressive disclosure on the read path** — the read-path guide (when to use memory, the quick pass with the memory tools, citation and write rules; citations are a native `memory_cite` tool call) is registered as a system-prompt section only while the session's store has a non-empty `memory_summary.md`, and that summary block is injected once per context window (redacted, injection-scanned, budget-capped). Codex parity: an empty store emits no memory instructions at all, not even the guide. Dynamic search hits are injected per pre-step.
...
- **Injection surface** — a plugin-sourced user message plus a system-prompt guide (DSH has no developer-role injection seam); Codex renders both pieces into one developer fragment. Both sides ship nothing when the store has no summary.
...
- **Note payload** — the write gate matches Codex ("only when explicitly asked"), but the shape differs: memcurio's `memory_remember` takes `content` + optional `kind` (remember|forget|update) and generates the note filename server-side, while Codex's `ad_hoc_note` takes a model-supplied filename plus the verbatim note.
- **Prompt composition** — the read-path guide and the Phase 1/2 prompts follow Codex's `read_path.md` / `stage_one_system.md` / `consolidation.md` section order, adapted to memcurio's seams (path-free layout, native tool calls, `memory_cite` instead of a citation block).
