...
- **Progressive disclosure on the read path** — the read-path guide (when to use memory, the quick pass with the memory tools, citation and write rules; citations are a native `memory_cite` tool call) is always registered as a system-prompt section, while the `memory_summary.md` block is injected only when the store actually has a summary (redacted, injection-scanned, budget-capped). Dynamic search hits are injected per pre-step.
