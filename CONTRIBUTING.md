...
- **Every change is audited**: all writes must go through `Transaction` (BEGIN/COMMIT/ROLLBACK) + audit; destructive operations (prune/curate/purge/repair) run in dry-run mode by default and only take effect with `--execute`.
