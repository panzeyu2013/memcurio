...
- After adding files or changing core paths, run: `bun test` (all green) + `bun run typecheck` (covers src/tests/scripts/client) + `bun run lint` (biome, zero diagnostics) + `bun run build` + `bun run pack:check`.
