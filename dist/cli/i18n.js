...
const zh = {
    "usage.main": `memcurio — 模型驱动的跨会话记忆系统（两阶段管线）
...
`,
    ...
    "status.root": (r) => `root: ${r}`,
    "status.config": (p, s) => `config: ${p} (${s})`,
    "status.configMissing": "缺失",
    "status.configOk": "正常",
    "status.configBroken": "损坏",
    "status.stage1": (p, s, d) => `stage1: pending=${p} selected=${s} deleted=${d}`,
    "status.notes": (t, p) => `ad-hoc notes: ${t} (pending=${p})`,
    "status.audit": (n) => `audit: ${n}`,
    "status.pending": (n) => `pending txns: ${n}`,
    "status.extractions": (n) => `提取任务 pending/processing/blocked: ${n}`,
    "remember.missing": "缺少记忆内容（用法: memcurio remember <内容>）",
    "remember.tooLong": (n) => `内容过长（上限 ${n} 字符）`,
    "remember.done": (f) => `已写入记忆 note: ${f}`,
    ...
    "purge.notFound": (k) => `未找到 rollout: ${k}`,
    "purge.done": (k, f, e, s) => `已物理清除 ${k}（artifact=${f}，清理 export 记录 ${e} 条，移除引用该 rollout 的 skill ${s} 个）；远端/其他备份仍需按保留策略处理`,
    "list.empty": "（暂无记忆）",
    ...
    "search.note": "无命中",
    "search.filtered": (n) => `${n} 条命中因注入模式被过滤`,
    "prune.none": "没有可剪除的 stage1 输出",
    "prune.applied": (n) => `已剪除 ${n} 条 stage1 输出（摘要文件已删除，MEMORY.md 已清理）`,
    ...
    "baseline.done": (dir, bytes) => `已注入 AGENTS.md (${dir}, ${bytes} bytes)`,
    "reindex.done": (n) => `已同步 ${n} 个文件并重置基线`,
    "repair.none": "事务日志健康，无需修复",
    "repair.pendingHeader": (n) => `发现 ${n} 个 pending 事务:`,
    "repair.corrupt": (n) => `发现 ${n} 行损坏日志`,
    "repair.generations": (n) => `发现 ${n} 个未收敛 generation manifest`,
    ...
    "repair.done": (n) => `已修复（重同步 ${n} 个文件，日志已清空）`,
    "doctor.layout": "布局",
    "doctor.config": "配置",
    "doctor.parsable": "可解析",
    "doctor.index": "索引库",
    ...
    "event.invalid": (e) => `事件无效: ${e}`,
    "export.done": (n, out) => `已导出 ${n} 条记录 -> ${out}`,
    "import.missing": "缺少导入文件（用法: memcurio import <file.jsonl>）",
    "import.empty": (p) => `${p} 中没有可导入的记录`,
    "import.tooLarge": (n) => `导入文件超过大小上限（${n} 字节）`,
    "import.invalid": (n) => `导入已中止：发现 ${n} 条无效或不安全记录（不会写入任何数据）`,
    "import.done": (a, s) => `导入完成: +${a}，跳过 ${s}`,
    "import.conflict": (k) => `跳过（已存在）: ${k}`,
    "extract.retryDone": (n, s, r, d, b, q) => `提取队列处理 ${n} 个：staged=${s} retry=${r} dead=${d} blocked=${b} requeued=${q}`,
    "setup.scopeGlobal": "全局（~/.config/opencode/opencode.json）",
    "setup.scopeProject": "项目（./opencode.json）",
    "setup.planPlugin": (s) => `opencode 插件: ${s}`,
    "setup.planMcp": (c) => `MCP server: ${c}`,
    "setup.dryRunHeader": "setup 计划（干跑；--apply 写盘）:",
    "setup.dryRunHint": "执行 memcurio setup --apply 写盘（先备份原文件为 .memcurio.bak）",
    "setup.applied": "已写入",
    "setup.noop": "无需改动",
    "setup.invalidJson": (f) => `配置不是合法 JSON: ${f}（请手工修复后重试）`,
    "setup.localMissing": (p) => `--source local 需要已构建的产物: ${p}（先运行 npm install && npm run build && npm run bundle:plugin）`,
    "setup.invalidSource": (v) => `无效的 --source: ${v}（可选 npm | github | local）`,
    ...
    "help.unknown": (c) => `未知命令: ${c}（memcurio help 查看命令列表）`,
    "version": (v) => `memcurio ${v}`,
    "note.extraArgs": (cmd, max) => `注意: ${cmd} 最多接受 ${max} 个位置参数，多余参数被忽略`,
    "note.invalidInt": (name, v, def) => `${name} 不是有效整数（${v}），使用默认值 ${def}`,
    "note.clamped": (name, v, max) => `${name}（${v}）超出上限，使用 ${max}`,
    "note.nonInteger": (name, v) => `${name}（${v}）不是整数，已取整`,
    "corruptIndex.hint": "index.sqlite 损坏：删除该文件后运行 memcurio reindex 重建（md 真源不受影响）",
};
const en = {
    "usage.main": `memcurio — model-driven cross-session memory (two-phase pipeline)
...
`,
    ...
    "help.doctor": "memcurio doctor\n  Self-check: layout/config/DB/workspace drift/pending txns",
    ...
    "status.audit": (n) => `audit: ${n}`,
    "status.pending": (n) => `pending txns: ${n}`,
    "status.extractions": (n) => `extraction jobs pending/processing/blocked: ${n}`,
    "remember.missing": "missing memory content (usage: memcurio remember <text>)",
    "remember.tooLong": (n) => `content too long (max ${n} chars)`,
    "remember.done": (f) => `memory note written: ${f}`,
    ...
    "purge.notFound": (k) => `rollout not found: ${k}`,
    "purge.done": (k, f, e, s) => `hard-purged ${k} (artifact=${f}, scrubbed ${e} export record(s), removed ${s} skill(s) referencing the rollout); remote/other backups still follow their retention policy`,
    ...
    "search.note": "no hits",
    "search.filtered": (n) => `${n} hit(s) filtered for injection patterns`,
    "prune.none": "nothing to prune",
    "prune.applied": (n) => `pruned ${n} stage-1 output(s); summaries removed and MEMORY.md cleaned`,
    ...
    "baseline.done": (dir, bytes) => `injected AGENTS.md (${dir}, ${bytes} bytes)`,
    "reindex.done": (n) => `synced ${n} file(s) and reset the baseline`,
    "repair.none": "transaction log healthy, nothing to repair",
    "repair.pendingHeader": (n) => `${n} pending transaction(s):`,
    "repair.corrupt": (n) => `${n} corrupt log line(s)`,
    "repair.generations": (n) => `${n} unfinished generation manifest(s)`,
    ...
    "repair.done": (n) => `repaired (re-synced ${n} file(s), log cleared)`,
    ...
    "event.invalid": (e) => `invalid event: ${e}`,
    "export.done": (n, out) => `exported ${n} record(s) -> ${out}`,
    "import.missing": "missing import file (usage: memcurio import <file.jsonl>)",
    "import.empty": (p) => `${p} contains no importable records`,
    "import.tooLarge": (n) => `import file exceeds the size limit (${n} bytes)`,
    "import.invalid": (n) => `import aborted: ${n} invalid or unsafe record(s); no data was written`,
    "import.done": (a, s) => `import done: +${a}, skipped ${s}`,
    "import.conflict": (k) => `skipped (already exists): ${k}`,
    "extract.retryDone": (n, s, r, d, b, q) => `processed ${n} extraction job(s): staged=${s} retry=${r} dead=${d} blocked=${b} requeued=${q}`,
    "setup.scopeGlobal": "global (~/.config/opencode/opencode.json)",
    "setup.scopeProject": "project (./opencode.json)",
    "setup.planPlugin": (s) => `opencode plugin: ${s}`,
    "setup.planMcp": (c) => `MCP server: ${c}`,
    ...
    "setup.invalidJson": (f) => `config is not valid JSON: ${f} (fix it manually and retry)`,
    "setup.localMissing": (p) => `--source local requires a built checkout: ${p} (run npm install && npm run build && npm run bundle:plugin first)`,
    "setup.invalidSource": (v) => `invalid --source: ${v} (choose npm | github | local)`,
    ...
    "error.prefix": "memcurio: ",
    "help.unknown": (c) => `unknown command: ${c} (memcurio help for the command list)`,
    "version": (v) => `memcurio ${v}`,
    "note.extraArgs": (cmd, max) => `note: ${cmd} accepts at most ${max} positional args; extra args ignored`,
    "note.invalidInt": (name, v, def) => `${name} is not a valid integer (${v}); using default ${def}`,
    "note.clamped": (name, v, max) => `${name} (${v}) exceeds the cap; using ${max}`,
    "note.nonInteger": (name, v) => `${name} (${v}) is not an integer; rounded`,
