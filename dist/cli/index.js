...
import { LlmLoopConsolidateProvider, RuleConsolidateProvider, planConsolidation, runConsolidation, syncArtifacts, withWorkspaceWriteLease, } from "../core/consolidate.js";
...
function warnExtraArgs(cmd, positionals, max) {
...
function positiveInt(value, fallback, max = 1000, name) {
    if (value === undefined) {
        return fallback;
    }
...
async function cmdBaseline(rest) {
...
async function cmdReindex(rest = []) {
...
function writeExportFile(path, text) {
    writeFileSync(path, text, { mode: 0o600 });
}
function safeImportScalar(value, max, required = false) {
...
function safeImportIso(value, fallback) {
...
function importCheckpointRank(sourceEvent) {
...
function validateImportRecord(value) {
    ...
    if (record.type === "stage1") {
        ...
        const status = record.status === undefined
            ...
            : record.status;
        if (!generatedAt || (record.lastUsage !== null && record.lastUsage !== undefined && !lastUsage) ||
...
async function cmdImport(rest) {
    ...
    return withStore(async (idx) => {
        ...
        const plannedNoteFiles = new Set();
        const additions = [];
        let skipped = 0;
...
async function cmdRetryExtraction(rest) {
    const { values, positionals } = parseArgs({
        args: rest,
        allowPositionals: true,
        options: {
