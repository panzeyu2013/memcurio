...
export function mcpCommandFor(source, opts) {
    if (source === "npm") {
...
export function opencodeConfigDir(configDir) {
    if (configDir) {
        return configDir;
    }
 ...
 *  <dir>/opencode.json. */
export function opencodeConfigFile(scope, opts) {
    if (scope === "global") {
...
export function pluginSpecFor(source, opts) {
    if (source === "npm") {
        return NPM_PACKAGE;
    }
    if (source === "github") {
...
function pluginAlreadyConfigured(plugin, spec) {
 ...
 *  memcurio never uses. */
export function planForFile(file, pluginSpec, mcpCommand, read) {
...
export function cmdSetup(rest) {
    ...
    const opts = { scope, source, plugin, mcp, apply, cwd: process.cwd(), mcpCommand };
    let plan;
    try {
        plan = buildPlan(opts);
    }
