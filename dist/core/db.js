...
function rowToExtractionJob(r) {
    const status = r.status === "processing" || r.status === "blocked" || r.status === "completed" || r.status === "dead"
        ? r.status
        : "pending";
    return {
        jobId: r.job_id,
...
export class Index {
    ...
    stageList() {
        return this.driver
            .all("SELECT * FROM stage1_outputs ORDER BY generated_at DESC")
            .map(rowToStage1);
    }
    ...
    stageArtifactFilenames() {
        return this.driver
            .all("SELECT artifact_filename FROM stage1_outputs WHERE artifact_filename IS NOT NULL")
    ...
    stagePruneRetention(batch = 200, maxUnusedDays) {
        ...
        const safeDays = maxUnusedDays !== undefined && maxUnusedDays > 0 && Number.isFinite(maxUnusedDays)
    ...
    noteList() {
        return this.driver
            .all("SELECT * FROM ad_hoc_notes ORDER BY created_at ASC")
            .map(rowToNote);
    }
    noteMarkApplied(ids) {
        for (const id of ids) {
    ...
    extractionClaim(provider, now = new Date().toISOString(), leaseMs = 120_000) {
    ...
    extractionRequeuePolicyRejected(provider, now = new Date().toISOString()) {
        this.driver.run(`UPDATE extraction_jobs
       SET status='pending', attempts=0, next_attempt_at=?, lease_until=NULL,
           claim_token=NULL, last_error=NULL, completed_at=NULL
       WHERE provider=? AND status='dead'
         AND last_error LIKE '%rejected by injection policy%'`, [now, provider]);
        return this.driver.get("SELECT changes() AS c")?.c ?? 0;
    }
    ...
    extractionFail(jobId, error, maxAttempts = 5, now = new Date().toISOString(), expectedClaimToken) {
        const row = expectedClaimToken === undefined
            ? this.driver.get("SELECT attempts FROM extraction_jobs WHERE job_id = ? AND status='processing'", [jobId])
            : this.driver.get("SELECT attempts FROM extraction_jobs WHERE job_id = ? AND status='processing' AND claim_token=? AND lease_until > ?", [jobId, expectedClaimToken, now]);
        if (!row) {
    ...
    extractionList(status) {
        const rows = status
            ? this.driver.all("SELECT * FROM extraction_jobs WHERE status=? ORDER BY created_at ASC", [status])
            : this.driver.all("SELECT * FROM extraction_jobs ORDER BY created_at ASC");
    ...
    extractionRenew(jobId, claimToken, now = new Date().toISOString(), leaseMs = 120_000) {
        const leaseUntil = new Date(Date.parse(now) + leaseMs).toISOString();
    ...
    extractionPruneCompleted(now = new Date().toISOString(), retentionDays = 30, maxRows = 10_000) {
    ...
    extractionPruneDead(now = new Date().toISOString(), retentionDays = 90, maxRows = 1_000) {
        const parsedNow = Date.parse(now);
        const cutoff = new Date((Number.isFinite(parsedNow) ? parsedNow : Date.now()) - retentionDays * 86_400_000).toISOString();
        this.driver.run(`DELETE FROM extraction_jobs
    ...
    extractionNextWakeAt(provider) {
        const row = provider === undefined
            ? this.driver.get(`SELECT MIN(CASE WHEN status='pending' THEN next_attempt_at ELSE lease_until END) AS next_at
    ...
    consolidationAcquire(leaseKey, owner, now = new Date().toISOString(), leaseMs = 900_000) {
        const leaseUntil = new Date(Date.parse(now) + leaseMs).toISOString();
        let acquired = false;
        this.withTransaction(() => {
    ...
    consolidationRenew(leaseKey, owner, now = new Date().toISOString(), leaseMs = 900_000) {
        const leaseUntil = new Date(Date.parse(now) + leaseMs).toISOString();
        let renewed = false;
        this.withTransaction(() => {
