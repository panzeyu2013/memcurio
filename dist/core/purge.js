...
import { renderRawMemories, removeBlocksCitingOnly, RuleConsolidateProvider, withWorkspaceWriteLease, } from "./consolidate.js";
...
function rawContainsManagedRow(raw, row) {
    ...
    return [...filenames].some((filename) => new RegExp(`## Rollout \\\`${escapeRegex(row.rolloutKey)}\\\`\\nupdated_at: [^\\n]+\\nrollout_summary_file: ${escapeRegex(filename)}\\n\\n${escapeRegex(body)}`).test(raw));
}
function scrubExportText(text, rolloutKey) {
    let removed = 0;
    const lines = [];
 ...
 * retention work. */
export function scrubExportFile(path, rolloutKey) {
...
export async function purgeRollout(root, rolloutKey, exportPaths = []) {
    return withWorkspaceWriteLease(root, async (idx, renew) => {
        ...
        try {
            ...
            const afterWorkspace = Object.fromEntries(Object.entries(beforeWorkspace).map(([rel, value]) => [rel, { ...value }]));
            ...
            const remainingRows = idx.stageList().filter((candidate) => candidate.rolloutKey !== rolloutKey &&
