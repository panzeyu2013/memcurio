...
export function createEvidenceSnapshot(inputs) {
    const items = [];
    let truncated = false;
    let redacted = false;
...
export class NoopExtractProvider {
    name = "noop";
    async extract() {
        return null;
    }
...
export function rolloutKeyFor(snapshot) {
...
export function buildExtractPrompt(snapshot) {
...
export function extractionIdempotencyKey(snapshot, sourceEvent, provider = "http") {
...
export function enqueueExtractionJob(idx, snapshot, sourceEvent, provider = "http") {
...
export async function queueExtraction(root, snapshot, sourceEvent, provider = "http") {
    ensureLayout(root);
...
export async function processExtractionQueue(root, provider, opts = {}) {
    ensureLayout(root);
...
export async function stageSession(root, snapshot, provider) {
    ensureLayout(root);
    const out = await provider.extract(snapshot);
    if (!out) {
        const idx = await Index.create(indexDb(root));
