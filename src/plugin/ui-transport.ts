...
function scopedDeltas(deltas: readonly ProjectedDelta[], session: string | undefined): ProjectedDelta[] {
  if (session === undefined) return [...deltas];
  return deltas.filter((delta) => {
    const deltaSession = (delta as { sessionId?: unknown }).sessionId;
    return deltaSession === undefined || deltaSession === session;
  });
}

function sseFrame(payload: unknown): string {
  return `id: ${String((payload as { seq: number }).seq)}\ndata: ${JSON.stringify(payload)}\n\n`;
}

interface TransportContextLike {
  inject(deps: string[], callback: (scoped: Context) => (() => void) | undefined): FiberLike;
 ...
 */
export function isSameOriginLoopbackRequest(req: IncomingMessage, url: URL, expectedToken: string): boolean {
  if (req.method !== "GET") return false;
  const remote = (req as { socket?: { remoteAddress?: string } }).socket?.remoteAddress;
  if (remote !== undefined && !LOOPBACK.has(remote)) return false;
  const host = req.headers.host;
  if (host === undefined) return false;
  const hostname = hostnameOf(host);
  if (hostname === undefined || !HOSTNAMES.has(hostname)) return false;
...
function parseSessionParam(url: URL): { sessionId?: string; malformed?: true } {
  const requested = url.searchParams.get("session");
  if (requested === null) return {};
  if (requested.length === 0 || requested.length > MAX_SESSION_ID) return { malformed: true };
 ...
 */
export function installUiTransport(ctx: Context, options: UiTransportOptions): UiTransport {
  ...
  const dropStream = (stream: ServerResponse): void => {
  ...
  const framePayload = (
    at: number,
    root: string,
    deltas: readonly ProjectedDelta[],
  ...
  const transport: UiTransport = {
    ...
    token,
    deliver(deltas, root) {
      if (disposed || deltas.length === 0 || root === undefined) return;
      ...
      for (const [stream, session] of [...streams]) {
        ...
        if (options.resolveRoot(session) !== root) continue;
    ...
    if (after !== undefined && streamRoot !== undefined) {
      ...
      if (oldestEntry !== undefined && after < oldestEntry.seq - 1) {
        ...
        for (const entry of history) {
          if (entry.seq <= after || entry.root !== streamRoot) continue;
          const payload = framePayload(entry.seq, entry.root, entry.deltas, parsed.sessionId);
          if (payload !== undefined) res.write(sseFrame(payload));
        }
      }
    }
  ...
  ctx.effect(() => {
    ...
    const bootstrap = (ctx as unknown as TransportContextLike).on("webserver/index-inject", (table) => {
      table.push({ kind: "global", name: UI_BOOT_GLOBAL, value: { basePath: UI_BASE_PATH, token } });
    });
    owned.push(bootstrap);
    const fiber = (ctx as unknown as TransportContextLike).inject(["webServer"], (scoped) => {
