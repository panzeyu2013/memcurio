...
function listAdoptableNoteFiles(root: string): string[] {
  let entries: Dirent[];
  try {
...
function timestampPrefix(filename: string): string {
 ...
 *  single bad row. */
function noteFilePathLenient(root: string, filename: string): string | null {
  try {
...
export async function pendingAdHocNotes(root: string, opts?: PendingAdHocNotesOptions): Promise<AdHocNote[]> {
  ...
  const remergeRejects: { filename: string; flag: string; hash: string }[] = [];
  ...
  if (settleMissing && remergeRejects.length) {
    ...
    try {
      idx.withTransaction(() => {
        ...
        try {
          known = new Map(Object.entries(JSON.parse(idx.metaGet(REJECTED_REMERGE_META) ?? "{}") as Record<string, unknown>).map(([k, v]) => [k, String(v)]));
        } catch {
