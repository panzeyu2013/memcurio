...
export function projectRawMemories(
  ...
  opts: { truncate?: boolean; afterKey?: string } = {},
): RawProjection {
  ...
  if (opts.afterKey !== undefined) {
    const start = sorted.findIndex((s) => s.rolloutKey.localeCompare(opts.afterKey as string) > 0);
    if (start > 0) {
...
function lastProjectedRolloutKey(root: string): string | undefined {
  let text: string;
  try {
...
function buildConsolidationSystemPrompt(input: ConsolidateInput, prunedResources: string[] = []): string {
  ...
  return [
    ...
    "- Scan for preferences first: surface user preferences and constraints before generic knowledge and place them high.",
