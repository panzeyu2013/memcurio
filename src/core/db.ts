...
export function addMissingColumns(
  ...
  try {
    const existing = new Set(driver.all<{ name: string }>(`PRAGMA table_info(${table})`).map((row) => row.name));
    for (const column of columns) {
      ...
      try {
        ...
        const now = new Set(driver.all<{ name: string }>(`PRAGMA table_info(${table})`).map((row) => row.name));
