...
export class LlmExtractProvider implements ExtractProvider {
  ...
  async extract(snapshot: RolloutSnapshot): Promise<Stage1Output | null> {
    ...
    try {
      const reply = await channel.agent(
...
export const EXTRACT_TOOLS: readonly ToolSpec[] = [
  {
    name: SAVE_EXTRACTION,
    description:
      "Save the durable memory extracted from this rollout. Call only when a future agent would plausibly act better because of it.",
    parameters: {
      type: "object",
      properties: {
        rollout_summary: {
          type: "string",
          description:
            "Task-structured recap with Outcome: success|partial|fail|uncertain per task, Preference signals, Key steps, Failures and how to do differently, Reusable knowledge, References.",
        },
        rollout_slug: {
          type: "string",
        ...
        raw_memory: {
          type: "string",
          description:
            "Raw memory with frontmatter (description/task/task_group/task_outcome/cwd/keywords) then '### Task N' blocks containing Preference signals / Reusable knowledge / Failures and how to do differently / References.",
        },
      },
  ...
  },
  {
    ...
    description:
      "Skip this rollout: nothing a future agent would act better on (one-off queries, generic status updates, temporary facts, common knowledge, no reusable steps or preferences).",
...
const EXTRACT_SYSTEM_PROMPT = [
  ...
  "",
  "You are a Memory Writing Agent. Your job: convert ONE agent rollout into useful raw memory",
  "and a rollout summary. The goal is to help future agents:",
  ...
  "- Raw rollout evidence is immutable: never edit it.",
  ...
  "Before writing, ask: \"Will a future agent plausibly act better because of what I write here?\"",
  ...
  "- no new artifacts, no new reusable steps, no preference or constraint likely to help later,",
  "then call skip_extraction instead of save_extraction.",
  "",
  "WHAT COUNTS AS HIGH-SIGNAL MEMORY",
  "High-signal memory changes the next agent's default behavior in a durable way:",
  "1. Stable user operating preferences — what the user repeatedly asks for, corrects, or",
  "   interrupts to enforce; what they want by default without restating it.",
  "2. High-leverage procedural knowledge — hard-won shortcuts, failure shields, exact",
  "   paths/commands, repo facts that save substantial future exploration.",
  "3. Reliable task maps and decision triggers — where the truth lives, how to tell a path is",
  "   wrong, and what signal should cause a pivot.",
  "4. Durable evidence about the user's environment and workflow — tooling habits, repo",
  "   conventions, presentation and verification expectations.",
  "Core principle: optimize for future USER time saved, not just future agent time.",
  "Non-goals: generic advice, secrets, copied large outputs, long recaps whose value is",
  "reconstructing the conversation, and exploratory discussion treated as settled memory.",
  "",
  "HOW TO READ A ROLLOUT (order of importance)",
  "1. User messages — strongest source for preferences, constraints, acceptance criteria,",
  "   dissatisfaction, and what should have been anticipated.",
  "2. Tool outputs / verification evidence — strongest source for repo facts, failures,",
  "   commands, exact artifacts, and what actually worked.",
  "3. Assistant actions/messages — reconstruction and steering; never the primary source of",
  "   truth for user preferences.",
  "Look for: repeated requests; corrections to scope, naming, ordering, visibility or",
  "presentation; points where the user stopped the agent or asked for a redo; requests a",
  "stronger agent could reasonably have anticipated; near-verbatim instructions that would",
  "make good defaults in future runs.",
  "",
  "TASK OUTCOME TRIAGE",
  "Classify EACH task in the rollout as success | partial | uncertain | fail.",
  "- success: task completed or correct result confirmed (explicit user feedback, tests, artifacts).",
  "- partial: meaningful progress, but incomplete, unverified or workaround only.",
  "- uncertain: no clear success/failure signal; the final task is usually uncertain without",
  "  explicit confirmation.",
  "- fail: not completed, wrong result, stuck loop, tool misuse, or user dissatisfaction.",
  "Explicit user feedback and environment/test validation outrank every heuristic. Repeated",
  "corrections, redo requests and interruptions are high-signal preference evidence too.",
  "",
  "DELIVERABLES",
  "Call exactly ONE of the provided tools:",
  "- save_extraction with rollout_summary, rollout_slug and raw_memory (field formats live in",
  "  the tool schema — follow them, including the per-task 'Outcome:' labels),",
  "- skip_extraction when the no-op gate says nothing is worth saving.",
  "Do not reply with prose or JSON outside the tool call. Write memory content in the LANGUAGE",
  "OF THE SOURCE conversation; only these instructions are English.",
  "",
  "WORKFLOW",
  "1. Apply the minimum-signal gate.",
  "2. Triage the outcome of every task.",
  "3. Read the rollout carefully: user messages, tool calls and outputs, assistant turns.",
  "4. Extract preference signals, reusable knowledge, failure shields and references.",
  "5. Make the single tool call.",
].join("\n");
...
export function parseExtractToolReply(
