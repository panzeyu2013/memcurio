...
import { indexDb, memoryRoot, nsDir } from "../src/core/paths.js";
...
describe("MemcoreAdapter", () => {
  test("sessionCreated registers session and state", async () => {
    ...
    const state = await adapter.sessionCreated("s1", "/tmp/MyProject");
    expect(state.ns).toBe("MyProject");
  ...
  test("idle within interval does not duplicate", async () => {
    ...
    await adapter.messageSeen("s1", "p1");
    await adapter.sessionIdle("s1");
    const before = readFileSync(join(nsDir(dir, "MyProject"), "SESSION.md"), "utf-8");
    await adapter.sessionIdle("s1");
    const after = readFileSync(join(nsDir(dir, "MyProject"), "SESSION.md"), "utf-8");
  ...
  test("reading a memory md file touches its entries", async () => {
    ...
    await adapter.sessionCreated("s1", "/tmp/MyProject");
    const idx = await Index.create(indexDb(dir));
    addEntry(nsDir(dir, "MyProject"), makeEntry());
    idx.add(makeEntry());
    idx.close();
    const memFile = join(nsDir(dir, "MyProject"), "MEMORY.md");
  ...
  test("buildCompactionContext contains namespace memory", async () => {
    ...
    const idx = await Index.create(indexDb(dir));
    addEntry(nsDir(dir, "MyProject"), entry);
    idx.add(entry);
    idx.close();
  ...
  test("sessionEnded writes final record and ends session row", async () => {
    ...
    idx.close();
    expect(existsSync(join(memoryRoot(dir), "MyProject", "SESSION.md"))).toBe(true);
  });
});
